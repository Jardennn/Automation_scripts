...
```bash
python pls.py -p <path> -a
```

The -p and -a flags are not required for running the script but do extend the functionality of the script.

## Todo and future features

* Date and time in the output
* More data about the entries in the output
* Sorting by File name (alphabetically and reverse, and by size)
